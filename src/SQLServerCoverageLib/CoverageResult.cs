...
namespace SQLServerCoverage
{
    public class CoverageResult : CoverageSummary
    {
        ...
        private readonly List<string> _sqlZeroHits;
        ...
        public void SaveSourceFiles(string path, bool skipZeroHit = false)
        {
            ...
            foreach (var batch in _batches)
            {
                if (skipZeroHit && batch.HitCount == 0)
                {
                    _sqlZeroHits.Add("-" + batch.ObjectName);
                    continue;
                }
        ...
        public void ToOpenCoverXml(string fullPath)
        {
            var rp = new OpenCoverXmlSerializer().Serialize(this);
            File.WriteAllText(fullPath, rp);
        }
            ...
            var reportConfiguration = new ReportConfigurationBuilder().Create(new Dictionary<string, string>()
                                            {
                                                ...
                                                { "VERBOSITY", "Info" },
                                                { "FileFilters", string.Join(";", _sqlZeroHits)}
